...
thread_exit (void)
{
  ...
  intr_disable ();
  /* Finds the child processes.
     If it has already terminated, deallocates its process descriptor.
     If it has not, separetes it from the parent.(disconnect the link.) */
	if (cur != initial_thread && !list_empty (&cur->child_list))
	{
      	...
      	; e = list_next (e))
  	{
  	  child = list_entry (e, struct thread, child_elem);
			...
			// set child's parent pointer to NULL. (disconnect the link)
        ...
        /* The child process has already terminated,
           so deallocates its process descriptor. */
	...
	/* Close all files. */
	for (i = 2; i <= cur->fdt_max; i++)
	{
		if (cur->fdt[i] != NULL)
			file_close (cur->fdt[i]);
	}

	/* If the parent process still alives,
     change the STATUS to THREAD_CHILD_WAIT
     , and nofity the parent process that this child process has terminated.*/
  if (cur->parent != NULL)
  {
...
/* Finds the process which has the thread id TID.
 * If the thread exists, return the pointer to its process descriptor.
...
/* Deallocates the child process's process descriptor. */
...
/* Disconnect the link to the child process. */
static void free_children (void)
{
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  sema_init (&t->sema_exec, 0);
	t->fdt_max = 1;  /* stdin=0, stdout=1 is already allocated.  */
