...
page_fault (struct intr_frame *f) 
{
  ...
  bool user;         /* True: access by user, false: access by kernel. */
	bool success = false;	/* True: success to load, false: fail to load. */
  ...
  /* If the virtual address is not presented in the page_table, */
  if (not_present)
  {
    /* Check if the memory reference is valid. */
    if (is_user_vaddr (fault_addr) && (fault_addr >= USER_VADDR_LOW_BOUND))
    {
      /* Locate the content that needs to go into the virtual memory page. */
      vme = find_vme (fault_addr);
      /* From the file / From the disk / All-zero page / Shared page */
      /* For shared page, the page can be in the page frame, but the in the page table. */
